...
class AthleteController extends Controller
{
    public function index($sportCode, $gameId)
    {
        ...
        $userArray = array();
        $tempIndividual = DB::table($sportCode.'_'.$gameId.'_'.$this->tableName)->distinct('u_id')->select('u_id')->get();
        foreach ($tempIndividual as $row) {
            array_push($userArray, $row->u_id);
        }
        $tempGroup = DB::table($sportCode.'_'.$gameId.'_'.$this->tableName)->leftJoin($sportCode.'_'.$gameId.'_teams', $sportCode.'_'.$gameId.'_teams.team_id', '=', $sportCode.'_'.$gameId.'_'.$this->tableName.'.team_id')->select($sportCode.'_'.$gameId.'_teams.team_id', $sportCode.'_'.$gameId.'_teams.member_list')->get();
