                ...
                if ($response['status'] == 'A02') { // check response
                    if (is_null($findUser)) { // add to user table
                        $temp = [
                            ...
                            'first_name_ch' => mb_substr(strval($response['name']), 1, NULL, 'UTF-8'),
                            'last_name_ch' => mb_substr(strval($response['name']), 0, 1, 'UTF-8'),
