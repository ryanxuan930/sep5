    ...
    for (let j = 0; j < res.length; j++) {
      if (res[j][`r${[input.round]}_heat`] == i + 1) {
        if (res[j][`r${[input.round]}_result`] != 'DNS' && res[j][`r${[input.round]}_result`] != 'DNF' && res[j][`r${[input.round]}_result`] != 'NM' && res[j][`r${[input.round]}_result`] != 'DQ') {
          if (input.multiple == 0) {
            data.push([res[j].u_id, res[j][`r${[input.round]}_lane`], `${res[j].bib != null ? '[' + res[j].bib + '] ' : ''}${res[j].last_name_ch} ${res[j].first_name_ch}`, `${res[j].first_name_en} ${res[j].last_name_en}`, res[j].org_name_ch]);
          } else {
