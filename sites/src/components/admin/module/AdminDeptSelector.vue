<script setup lang="ts">
  ...
  const adminDeptList: any = ref(null);
  ...
  <div>
    <table>
      ...
      <template v-for="(item, index) in adminDeptList" :key="index">
        <tr>
          <td>
            <input :value="item.admin_dept_id" type="checkbox" name="game_id" v-model="selectedData">
          </td>
          <td>[{{ item.admin_org_name_ch }}] {{ item.admin_dept_name_ch }}</td>
        </tr>
      </template>
    </table>
