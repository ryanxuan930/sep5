<script setup lang="ts">
  ...
  import EditAdminOrg from '@/components/admin/account/EditAdminOrg.vue';
  ...
  async function getDataList(url = 'admin/admin-org') {
  ...
  <div id="section-box" ref="boxRef">
    <table>
      ...
      <template v-for="(item, index) in dataList" :key="index">
        <tr>
          <td :style="{'width': `${boxWidth*0.1}px`}">{{ item.admin_org_id }}</td>
          <td>
            <div :style="{'width': `${boxWidth*0.4}px`}">{{ item.admin_org_name_ch }}</div>
          </td>
          <td>
            <div :style="{'width': `${boxWidth*0.35}px`}">{{ item.org_name_full_ch }}</div>
          </td>
    ...
    <FullModal v-show="displayModal" @closeModal="displayModal = false">
      ...
      <template v-slot:content>
        <EditAdminOrg v-if="displayModal" :input-data="selectedData" @refreshPage="getDataList()" @closeModal="displayModal = false"></EditAdminOrg>
      </template>
    </FullModal>
  </div>
</template>
