...
async function addEvent(input: any) {
  ...
  for (const data of groupList.value) {
    if (data.division_id == input.division_id && data.event_code == input.event_code && data.team_id == input.team_id) {
  ...
  const response = await vr.Post(`game/${route.params.sportCode}/${route.params.gameId}/common/group`, data, null, true, true);
