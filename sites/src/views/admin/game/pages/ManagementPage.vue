  ...
  <div v-if="gameData != null" class="flex flex-col h-full gap-5">
    ...
    <div class="section-box flex-grow h-full flex flex-col gap-4 overflow-hidden">
      ...
      <div class="flex-grow h-full overflow-hidden">
        <ScheduleList v-if="gameData.module == 'ln' || gameData.module == 'rd'" :displayMode="'call'"></ScheduleList>
      </div>
    </div>
  </div>
  <FullModal v-show="displayModal > 0" @closeModal="displayModal = 0">
    ...
    <template v-slot:content>
      <div class="overflow-auto h-full">
        <ScheduleList v-if="displayModal == 1" :displayMode="'management'"></ScheduleList>
      </div>
    </template>
  </FullModal>
</template>
